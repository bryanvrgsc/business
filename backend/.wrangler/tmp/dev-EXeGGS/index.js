...
// .wrangler/tmp/bundle-7xAiW5/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-7xAiW5/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-7xAiW5/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var getDb = /* @__PURE__ */ __name((env2) => {
  ...
  const client = new Client({
    ...
    ssl: env2.DATABASE_URL ? false : void 0
  });
  return client;
}, "getDb");
...
auth.post("/login", async (c) => {
  ...
  try {
    ...
    if (user.password_hash !== "hashed_admin") {
      if (password !== "admin") {
    ...
    const token = await sign3(payload, JWT_SECRET);
    return c.json({
      token,
      user: {
        id: user.id,
...
// src/middleware.ts
var JWT_SECRET2 = "your-secret-key-change-this";
var authMiddleware = createMiddleware(async (c, next) => {
  ...
  try {
    const payload = await verify3(token, JWT_SECRET2, "HS256");
    c.set("user", payload);
    await next();
  } catch (e) {
...
app.get("/", (c) => {
  return c.text("CMMS Backend is running \u{1F680}");
});
...
app.get("/seed", async (c) => {
  ...
  try {
    ...
    await client.query(`
        ...
        `);
    await client.end();
    return c.json({ message: "Database seeded successfully! \u{1F331}" });
  } catch (e) {
...
app.get("/api/forklifts/:id", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      // TODO: Join with locations table
      nextMaintenance: "2024-03-01",
      // TODO: Calculate from schedules
      image: "/forklift-placeholder.png"
      // TODO: R2 Image
    });
  } catch (e) {
...
app.post("/api/sync", async (c) => {
  ...
  const report2 = await c.req.json();
  const { forkliftId, templateId, answers, hasCriticalFailure, capturedAt } = report2;
